...
var addFunctions = function (context) { return function (rootNode) {
    function visit(node) {
            ...
            ts.setSyntheticLeadingComments(node, [
                {
                    ...
                    text: "The below JavaScript class is an auto generated code for NodeCloud Azure plugin, Please do not change",
...
var addIdentifiers = function (context) { return function (rootNode) {
    var count = 0;
    var clientCount = 0;
    function visit(node) {
        ...
        if (ts.isIdentifier(node) && dummyIdentifiers.includes(node.text)) {
            ...
            switch (node.text) {
                case "ClassName":
                    updatedIdentifier = ts.updateIdentifier(ts.createIdentifier(classData.functions[0].pkgName.split("-")[1]));
                    break;
                case "SDKClassName":
                    updatedIdentifier = ts.updateIdentifier(ts.createIdentifier(classData.functions[count].fileName.split(".")[0]));
                    break;
                case "functionClient":
                    updatedIdentifier = ts.updateIdentifier(ts.createIdentifier(classData.functions[count].client));
                    break;
                case "ClientName":
                    updatedIdentifier = ts.updateIdentifier(ts.createIdentifier(clients[clientCount]));
                    clientCount++;
                    break;
...
function transform(code, data) {
    ...
    // import related
    clients = Array.from(new Set(classData.functions.map(function (method) { return method.client; })));
    ...
    code = ts.createSourceFile("azureDummyClass.js", outputOne, ts.ScriptTarget.Latest);
