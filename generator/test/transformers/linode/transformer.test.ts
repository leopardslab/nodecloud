...
describe("Linode transformer transform", () => {
  context("Valid source code and valid data", () => {
    const testData: TestData = { AST: null, data: null };
    before(async () => {
      testData.AST = await readSourceFile("validDataset", "linode");
      testData.data = await readJsonData("validDataset", "linode");
    });
  ...
  context("Invalid source code and valid data", () => {
    const testData: TestData = { AST: null, data: null };
    before(async () => {
      testData.AST = await readSourceFile("invalidDataset_1", "linode");
      testData.data = await readJsonData("invalidDataset_1", "linode");
    });
  ...
  context("Valid source code and invalid data", () => {
    const testData: TestData = { AST: null, data: null };
    before(async () => {
      testData.AST = await readSourceFile("invalidDataset_2", "linode");
      testData.data = await readJsonData("invalidDataset_2", "linode");
    });
