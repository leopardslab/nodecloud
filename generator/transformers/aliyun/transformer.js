...
var addMultiLineComment = function (node, comment) {
    ts.addSyntheticLeadingComment(node, ts.SyntaxKind.MultiLineCommentTrivia, comment, true);
};
var runTransformation = function (sourceCode, transformMethod) {
...
var toSourceFile = function (sourceCode) {
    return ts.createSourceFile('dummyClass.js', sourceCode, ts.ScriptTarget.Latest, true);
};
var extractParams = function (classData, identifierCount) {
    var parameters = classData.functions[identifierCount].params.map(function (param) {
...
var extractArgs = function (classData, identifierCount) {
    var args = classData.functions[identifierCount].params.map(function (param) {
        return ts.createIdentifier(param.name);
    });
    return args;
};
exports.transform = function (code, classData) { return __awaiter(void 0, void 0, void 0, function () {
        ...
        switch (_a.label) {
            case 0:
                addFunctions = function (context) { return function (rootNode) {
                    var visit = function (node) {
                        if (ts.isClassDeclaration(node)) {
                            var functions = classData.functions.map(function (method) {
                                // const clonedNode = Object.assign({}, node.members[1]);
                                var clonedNode = __assign({}, node.members[1]);
                                clonedNode.name = ts.createIdentifier(method.functionName);
                                return clonedNode;
                            });
                            // console.log(functions);
                            var updatedClass = ts.updateClassDeclaration(node, node.decorators, node.modifiers, node.name, node.typeParameters, node.heritageClauses, ts.createNodeArray([node.members[0]].concat(functions)));
                            return updatedClass;
                        }
                ...
                addIdentifiers = function (context) { return function (rootNode) {
                    ...
                    var visit = function (node) {
                        if (ts.isMethodDeclaration(node)) {
                            var parameters = extractParams(classData, count);
                        ...
                        if (ts.isIdentifier(node) && dummyIdentifiers.includes(node.text)) {
                            ...
                            switch (node.text) {
                                case 'ClassName':
                                    updatedIdentifier = ts.updateIdentifier(ts.createIdentifier('ALI_' + classData.serviceName));
                                    break;
                                case '_sdkClassName':
                        ...
                        if (ts.isCallExpression(node)) {
                            node.expression.forEachChild(function (childNode) {
                                if (ts.isIdentifier(childNode) &&
                                    ...
                                    // );
                                    var args = extractArgs(classData, count);
                ...
                addComments = function (context) { return function (rootNode) {
                    ...
                    var visit = function (node) {
                        if (ts.isClassDeclaration(node)) {
                            addMultiLineComment(node, 'This is an auto generated class, please do not edit this file!');
