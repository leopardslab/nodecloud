							...
							function visit(node) {
								if (ts.isMethodDeclaration(node)) {
									...
									params = params.concat(
										data.functions[count].params
									);
									data.functions[
										count
									].allParams = data.functions[
										count
									].allParams.concat(params);
									var parameters = params.map(function(
										param
									) {
									...
									node.parameters = parameters.concat(
										node.parameters
									);
								}
								if (
					...
					addComments = function(context) {
						return function(rootNode) {
							...
							function visit(node) {
								...
								if (ts.isMethodDeclaration(node)) {
									var parameters = data.functions[
										count
									].allParams.map(function(param) {
										var statment;
										if (param.optional) {
											statment =
												'* @param {' +
