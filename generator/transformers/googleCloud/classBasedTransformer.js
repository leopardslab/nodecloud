                                    ...
                                    case '_client':
                                        if ((data.functions[count].classConstructorData.parameters[0].type =
                                            'TypeReference' &&
                                                !data.functions[count].classConstructorData
                                                    .parameters[0].optional)) {
                                            updatedIdentifier = ts.updateIdentifier(ts.createIdentifier(data.functions[count].classConstructorData
                                                .parameters[0].name));
                                        }
                                        else {
                        ...
                        function visit(node) {
                            ...
                            if (ts.isMethodDeclaration(node)) {
                                var parameters = data.functions[count].allParams.map(function (param) {
                                    var statment;
                                    if (param.optional) {
                                        statment = "* @param {" + (param.typeRefName
                                            ? param.typeRefName
                                            : param.type) + "} [" + param.name + "] - Optional parameter";
                                    }
                                    else {
                                        statment = "* @param {" + (param.typeRefName
                                            ? param.typeRefName
                                            : param.type) + "} " + param.name + " - Mandatory parameter";
                                    }
                                    return statment;
                                });
                                var comment = void 0;
